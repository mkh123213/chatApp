...
import 'package:chat_material3/core/common/widgets/chat/disappearing_messages_dialog.dart';
...
import 'package:chat_material3/features/groups/presentation/bloc/group_typing_cubit/group_typing_cubit.dart';
...
class SelectedGroupChatScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider<SelectedGroupChatCubit>(
          create: (_) => sl<SelectedGroupChatCubit>(),
        ),
        BlocProvider<GroupTypingCubit>(
          create: (_) =>
              sl<GroupTypingCubit>()..watchTyping(groupId: group.id),
        ),
      ],
      child: SafeArea(
        child: Column(
...
class _GroupChatHeader extends StatelessWidget {
  ...
  Widget _buildNormalHeader(BuildContext context) {
    ...
    return ChatAppBar(
      title: group.name,
      subtitle: BlocBuilder<GroupTypingCubit, GroupTypingState>(
        builder: (context, typingState) {
          final typingCubit = context.read<GroupTypingCubit>();
          if (typingCubit.isAnyoneTyping(currentUserId)) {
            return Text(
              typingCubit.getTypingText(currentUserId),
          ...
          return TextApp(
            text:
                '${group.members.length} ${context.translate(LangKeys.members)}',
          ...
          onSelected: (value) {
            if (value == 'disappearing') {
              showDisappearingMessagesDialog(
                context: context,
                currentDuration: group.disappearingDuration,
                onSelect: (duration) {
                  sl<GroupsRepo>().setDisappearingMessages(
                        groupId: group.id,
                        durationInSeconds: duration,
                      );
                  ShowToast.showToastSuccessTop(
                    message: context.translate(LangKeys.disappearingSet),
                  );
                },
              );
            }
            ...
            PopupMenuItem(
              ...
              child: Row(
                children: [
                  ...
                  Text(context.translate(LangKeys.disappearingMessagesSetting)),
                ],
              ),
            ),
          ],
