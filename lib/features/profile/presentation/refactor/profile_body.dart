...
class _OnlineVisibilityTile extends StatefulWidget {
  @override
  State<_OnlineVisibilityTile> createState() => _OnlineVisibilityTileState();
}

class _OnlineVisibilityTileState extends State<_OnlineVisibilityTile> {
  ...
  Future<void> _loadVisibility() async {
    ...
    if (mounted) {
      setState(() {
        _visibility = data?['onlineVisibility'] as String? ?? 'everyone';
        _loaded = true;
      });
    }
  }
  ...
  Future<void> _setVisibility(String value) async {
    ...
    final repo = sl<UserPresenceRepo>();
    await repo.setOnlineVisibility(
  ...
  @override
  Widget build(BuildContext context) {
    final label = _visibility == 'nobody'
        ? context.translate(LangKeys.onlineVisibilityNobody)
        : context.translate(LangKeys.onlineVisibilityEveryone);

    return ListTile(
      leading: Icon(Icons.visibility_outlined, color: context.color.onSurface),
      title: Text(
        context.translate(LangKeys.onlineVisibility),
        style: TextStyle(fontSize: 15.sp),
      ),
      subtitle: _loaded
          ? Text(
          ...
          : null,
      trailing:
          Icon(Icons.chevron_right, color: context.color.onSurfaceVariant),
      onTap: () {
