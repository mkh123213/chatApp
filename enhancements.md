...
**What**: Tap a message → "Reply" → shows quoted message above the input bar. The reply message includes `replyToId`, `replyToText`, `replyToType`.
**Display**: Show a small preview card above the message bubble with the original text/image.
**Files**: New fields in `MessageModel`, update `message_input_bar.dart`, `message_bubble.dart`
**Effort**: Medium
...
### 16. Notification Tap Deep Linking
...
### 17. Delivered Status (Double Check)
...
### 18. Voice/Video Call for Groups
...
### 19. Typing Indicator in Group Chat
**Current**: Typing indicators exist for single chats but not groups.
**Fix**: Extend the typing system to groups. Show "Ahmed is typing..." or "Ahmed, Mohamed are typing..." in the group chat.
**Effort**: Low-Medium

### 20. Message Seen By (Group)
**Current**: Groups have `readBy` array but no UI to show who has seen a message.
**Fix**: Long-press a sent message → "Seen by" → show list of members who read it with timestamps.
**Effort**: Low

### 21. Swipe to Reply
**What**: Swipe right on a message to trigger reply (like WhatsApp).
**How**: Wrap message bubble in a `Dismissible` or custom swipe gesture.
**Effort**: Low

### 22. Chat Pinning and Archive UI
**Current**: `pinChat`/`archiveChat` methods exist in data source but may not be fully wired to UI.
**Fix**: Ensure pinned chats appear at the top of the list. Add an "Archived" section at the bottom. Long-press → "Pin" / "Archive" options.
**Effort**: Low-Medium

### 23. Online/Last Seen Privacy Setting
**What**: Let users choose who can see their online status:
- Everyone
- My contacts only
- Nobody
**How**: Add `onlineVisibility` field to user document. Check it before showing presence status.
**Effort**: Low

### 24. Disappearing Messages
**What**: Chat-level setting where messages auto-delete after 24h / 7 days / 30 days.
**How**: Add `disappearingDuration` to chat document. Use a Cloud Function with scheduled trigger or client-side check on load.
**Effort**: Medium

### 25. Multi-Select Messages
**What**: Long-press → select multiple messages → delete/forward selected.
**How**: Add selection mode to `MessagesCubit`. Show checkboxes on message bubbles. Batch operations on selected IDs.
**Effort**: Medium

### 26. Contact Sync
**Current**: Users find each other by email search only.
**Fix**: Use `flutter_contacts` to read the phone book, match phone numbers against Firestore `users` collection, and show "Contacts on ALKHATEEB CHAT" section in new chat screen.
**Why**: This is how WhatsApp/Telegram discover users — much easier than typing emails.
**Effort**: Medium
...
### 27. Remove `domain/` Layer Violation
...
### 28. Fix Typo: `fierstore` → `firestore`
...
### 31. Widget Tests for Critical Screens
...
## Priority 6: Scalability & Backend

### 33. Move Notification Fan-Out to Server
...
### 34. Server-Side Stale Call Cleanup
...
### 35. Status Auto-Expiry via Server
...
### 36. Firestore Offline Persistence
...
### 37. Rate Limiting on Edge Functions
...
### 41. Custom Chat Themes Per Conversation
...
### 45. Audio/Video Recording in Chat
...
| 20 | Message "Seen by" UI for groups | 1-2 hours |
| 21 | Swipe to reply gesture | 1-2 hours |
| 23 | Online/last seen privacy setting | 1-2 hours |
| 27 | Remove `domain/` layer violation | 30 min |
| 32 | Clean up dead code | 1 hour |
| 37 | Rate limiting on Edge Functions | 1 hour |
| 41 | Per-chat wallpaper | 1-2 hours |
